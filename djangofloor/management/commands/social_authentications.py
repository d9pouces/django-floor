...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser):
        parser.add_argument('action', help='show: display configured authentications,\n'
    ...
    def add_provider(self):
        ...
        choices = OrderedDict([(str(x + 1), y)
                               for (x, y) in enumerate(sorted(providers.values(), key=lambda x: x.provider_name))])
        # select a provider
        ...
        while True:
            ...
            if choice == '0':
                ...
                self.stderr.write('Invalid choice (%s). Please enter a valid choice.' % choice)
                continue
        ...
        if section in loaded_configurations:
            self.stderr.write('%s is already defined. Overwrite its configuration? y/[N] ')
            if input().lower() != 'y':
                return
        ...
        self.stderr.write('Write the configuration to %s? y/[N] ' % settings.ALLAUTH_APPLICATIONS_CONFIG)
        if input().lower() != 'y':
            return
        try:
            ...
            self.stdout.write(self.style.SUCCESS('Do not forget to run the "migrate" command to load '
                                                 'this configuration.'))
        except Exception as e:
            self.stderr.write('Unable to write %s (%s).' % (settings.ALLAUTH_APPLICATIONS_CONFIG, e))

    def show_config(self):
        providers = get_loaded_configurations()
        self.stdout.write(self.style.SUCCESS('configuration file: %s' % settings.ALLAUTH_APPLICATIONS_CONFIG))
        self.stdout.write(self.style.SUCCESS('%d provider(s) found' % len(providers)))
        for provider in providers.values():
            self.stdout.write(self.style.SUCCESS('provider: %s' % provider.provider_name))
        ...
        if migrate(read_only=True):
            self.stdout.write(self.style.WARNING('Do not forget to run the "migrate" command to load '
                              'this configuration.'))
        else:
            self.stdout.write(self.style.SUCCESS('database representations of these providers are valid.'))
