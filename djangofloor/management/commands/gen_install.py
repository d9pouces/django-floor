...
from djangofloor.conf.settings import merger as original_merger
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        providers = original_merger.providers
        if options['default_conf']:
            providers = list(providers) + [IniConfigProvider(options['default_conf'])]
        merger = SettingMerger(fields_provider=original_merger.fields_provider,
                               providers=providers, extra_values=original_merger.extra_values)
        merger.process()
        merger.post_process()
        ...
        template_values = {'username': options.get('user', merger.settings['DF_MODULE_NAME']),
                           'bind_address': merger.settings['LISTEN_ADDRESS'],
                           'project_name': merger.settings['DF_PROJECT_NAME'],
