...
class Command(TemplatedBaseCommand):
    ...
    def build_package(self, package_type):
        ...
        if package_src_filename:
            ...
            self.stdout.write(
                self.style.SUCCESS("package %s created" % package_dst_filename)
            )
            self.template_context["package_filename"] = package_basename
            self.copy_vagrant_script(
                "djangofloor/vagrant/run_package.sh", execute=False
            )
        else:
    ...
    def get_template_context(self, merger, extra_context):
        ...
        context["fpm_default_config_filename"] = (
            self.host_fpm_default_config_filename,
            self.vagrant_fpm_default_config_filename,
        )
        context["fpm_project_config_filename"] = (
            self.host_fpm_project_config_filename,
            self.vagrant_fpm_project_config_filename,
        )
        context["fpm_custom_config_filename"] = (
            self.host_fpm_custom_config_filename,
    ...
    def get_fpm_command_line(self, package_type):
        ...
        for ini_option, cli_option in FPM_CLI_OPTIONS.items():
            ...
            if cli_option in FPM_BOOL_OPTIONS:
                ...
                filename = os.path.join(
                    self.host_tmp_dir, "%s.%s" % (os.path.basename(template_name), sha1)
                )
                with open(filename, mode="w", encoding="utf-8") as fd:
                    fd.write(content)
                filename = os.path.join(
                    self.vagrant_tmp_dir,
                    "%s.%s" % (os.path.basename(template_name), sha1),
                )
    ...
    def update_template_context(self, package_type):
        ...
        for option_name in parser.options("processes"):
            ...
            process_categories[values["attr"]] = os.path.join(
                self.vagrant_install_dir, "bin", option_name
            )
        processes = {
