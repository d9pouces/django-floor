...
class TemplatedBaseCommand(OriginalBaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        remove_arguments_from_help(parser, {'--settings', '--traceback', '--pythonpath', '--no-color'})
    ...
    @classmethod
    def browser_folder(cls, src_module, src_folder, searched_folder='templates/', context=None):
            ...
            for (root, dirnames, filenames) in walk(src_module, searched_base):
                for filename in filenames:
                    ...
                    if src_path.endswith(cls.transparent_suffix):
                        dst_path = src_path[filename_root_len:-len(cls.transparent_suffix)]
                    else:
                        dst_path = src_path[filename_root_len:]
                    if context and '{' in dst_path:
                        format_values = {}
                        for (literal_text, field_name, format_spec, conversion) in formatter.parse(dst_path):
    ...
    def get_file_writers(self, searched_locations, context=None):
        ...
        for (module_name, folder_name) in searched_locations:
            msg = 'Looking for static files in %s:[static/]%s' % (module_name, folder_name)
            self.stdout.write(self.style.SUCCESS(msg))
            new_templates = self.browser_folder(module_name, folder_name, searched_folder='static/', context=context)
            for target_filename, source_filename in new_templates.items():
                writers[target_filename] = StaticWriter(target_filename, source_filename, stdout=self.stdout,
                                                        style=self.style)
            msg = 'Looking for template files in %s:[templates/]%s' % (module_name, folder_name)
            self.stdout.write(self.style.SUCCESS(msg))
            new_templates = self.browser_folder(module_name, folder_name, searched_folder='templates/', context=context)
            for target_filename, template_filename in new_templates.items():
                writers[target_filename] = TemplateWriter(target_filename, template_filename, stdout=self.stdout,
                                                          style=self.style)
        return writers
...
class Writer:
    ...
    def write(self, target_directory=None, context=None, dry_mode=False, verbose_mode=False):
        raise NotImplementedError
