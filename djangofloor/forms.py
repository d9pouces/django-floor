...
class LogNameForm(forms.Form):
    log_name = forms.ChoiceField(required=False, initial=None, label=_('configured logger'),
                                 choices=[(x, x) for x in sorted(settings.LOGGING['loggers'])])
    other_log_name = forms.CharField(max_length=255, min_length=1, label=_('other logger name'), required=False,
                                     help_text=_('Please enter the name of another logger to check.'))
    message = forms.CharField(max_length=255, min_length=1, label=_('message'), initial=_('log test'))
    level = forms.ChoiceField(label=_('level'), initial=40,
