...
sudo -H -u "{{ DF_MODULE_NAME }}" "{{ process.control.command_line }}" migrate
...
EOF
cat << EOF | sudo -H -u "{{ DF_MODULE_NAME }}" "{{ process.control.command_line }}" shell
...
sudo -H -u "{{ DF_MODULE_NAME }}" "{{ processes.{% if USE_CELERY %}.command_line }}" server
