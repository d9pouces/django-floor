...
urlpatterns = [
    url(prefix + r"jsi18n/$", catalog_view, name="jsi18n"),
    url(
        prefix + r"%s(?P<path>.*)$" % settings.MEDIA_URL[1:],
        serve,
        {"document_root": settings.MEDIA_ROOT},
    ),
    url(
        prefix + r"%s(?P<path>.*)$" % settings.STATIC_URL[1:],
        serve,
    ...
    url(
        prefix + r"apple-touch-icon\.png$",
        serve,
        {"document_root": settings.STATIC_ROOT, "path": "favicon/apple-touch-icon.png"},
    ),
    url(
        prefix + r"apple-touch-icon-precomposed\.png$",
        serve,
        {
            "document_root": settings.STATIC_ROOT,
...
if settings.DF_INDEX_VIEW:
    urlpatterns += [
        url(prefix + r"$", get_view_from_string(settings.DF_INDEX_VIEW), name="index")
    ]
